...
) : Command()
{
    ...
    )
    {
        ...
        if (conversationProperties.EndMeetingSchedule < 0)
        {
            return;
        }
        if (conversationProperties.EndMeetingSended is true)
        {
            return;
        }
        var endTime = conversationProperties.ScheduledEndTime;
        if (endTime is null)
        {
            return;
        }
        ...
        if ((int)timeSpan.TotalMinutes > conversationProperties.EndMeetingSchedule)
        {
            return;
        }
        try
        {
            this.logger.EndMeetingMessageNotifying(
                ...
                message: conversationProperties.EndMeetingMessage,
                url: conversationProperties.EndMeetingUrl
            );
            var card = new AdaptiveCard("1.3");
            if (string.IsNullOrEmpty(conversationProperties.EndMeetingMessage) is false)
            {
                card.Body.Add(
        ...
        conversationProperties.EndMeetingSended = true;
    }
