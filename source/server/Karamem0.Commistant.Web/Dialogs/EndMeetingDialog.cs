...
) : ComponentDialog
{
    ...
    private async Task<DialogTurnResult> OnBeforeAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken = default)
    {
        ...
        var options = (ConversationPropertiesOptions?)stepContext.Options;
        var value = this.mapper.Map(
            options,
            conversationProperties with
            {
            }
        );
    ...
    private async Task<DialogTurnResult> OnAfterAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken = default)
    {
        ...
        {
            conversationProperties.EndMeetingSchedule = value.Value("Schedule", -1);
            conversationProperties.EndMeetingMessage = value.Value<string>("Message", null);
            conversationProperties.EndMeetingUrl = value.Value<string>("Url", null);
        ...
        if (stepContext.Context.Activity.ReplyToId is not null)
        {
                        ...
                        [
                            ...
                            new AdaptiveColumn()
                            {
                                ...
                                [
                                    new AdaptiveTextBlock()
                                    {
                                        Text = conversationProperties.EndMeetingSchedule switch
                                        {
                                            -1 => "なし",
                                            0 => "予定時刻",
                                            _ => $"{conversationProperties.EndMeetingSchedule} 分前"
                                        }
                                    }
                                ],
                                Width = "stretch"
                        ...
                        [
                            ...
                            new AdaptiveColumn()
                            {
                                ...
                                [
                                    new AdaptiveTextBlock()
                                    {
                                        Text = $"{conversationProperties.EndMeetingMessage}",
                                        Wrap = true
                                    }
                                ],
                                Width = "stretch"
                        ...
                        [
                            ...
                            new AdaptiveColumn()
                            {
                                ...
                                [
                                    new AdaptiveTextBlock()
                                    {
                                        Text = $"{conversationProperties.EndMeetingUrl}"
                                    }
                                ],
                                Width = "stretch"
                            }
            ...
            if (Uri.TryCreate(
                    conversationProperties.EndMeetingUrl,
                    UriKind.Absolute,
                    out var url
                ))
            {
