...
public class MeetingEndingDialog(
    ConversationState conversationState,
    IMapper mapper,
    ILogger<MeetingEndingDialog> logger
) : ComponentDialog
{
    ...
    private readonly ILogger<MeetingEndingDialog> logger = logger;
    ...
    private async Task<DialogTurnResult> OnBeforeAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken = default)
    {
        ...
        var editCardData = this.mapper.Map<MeetingEndingEditCardData>(
            this.mapper.Map(
                commandOptions,
        ...
        var editCard = MeetingEndingEditCard.Create(editCardData);
    ...
    private async Task<DialogTurnResult> OnAfterAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken = default)
    {
        ...
        var response = JsonConverter.Deserialize<MeetingEndingResponse>(value);
        if (response is null)
        {
        ...
        {
            var viewCardData = this.mapper.Map<MeetingEndingViewCardData>(commandSettings);
            var viewCard = MeetingEndingViewCard.Create(viewCardData);
            var activity = MessageFactory.Attachment(
                new Attachment()
                {
                    ContentType = "application/vnd.microsoft.card.adaptive",
    ...
    public class MapperConfiguration(IQRCodeService qrCodeService) : IRegister
    {
        ...
        {
            _ = config
                .NewConfig<CommandSettings, MeetingEndingEditCardData>()
                .Map(d => d.Schedule, s => s.MeetingEndingSchedule)
                .Map(d => d.Message, s => s.MeetingEndingMessage ?? "")
                .Map(d => d.Url, s => s.MeetingEndingUrl ?? "");
            _ = config
                .NewConfig<CommandSettings, MeetingEndingViewCardData>()
                .Map(d => d.Message, s => s.MeetingEndingMessage ?? "")
                .Map(d => d.Url, s => s.MeetingEndingUrl ?? "")
                .Map(d => d.QrCode, s => "")
                .AfterMapping(async (s, d) =>
                    {
                        d.Schedule = s.MeetingEndingSchedule switch
                        {
                            -1 => "なし",
                            0 => "予定時刻",
                            _ => $"{s.MeetingEndingSchedule} 分前"
                        };
                        if (Uri.TryCreate(
                                s.MeetingEndingUrl,
                                UriKind.Absolute,
            ...
            _ = config
                .NewConfig<MeetingEndingResponse, CommandSettings>()
                .Map(d => d.MeetingEndingSchedule, s => s.Schedule)
                .Map(d => d.MeetingEndingMessage, s => s.Message)
                .Map(d => d.MeetingEndingUrl, s => s.Url);
        }
