...
) : TeamsActivityHandler
{
    ...
    )
    {
        foreach (var member in membersAdded)
        {
            ...
            {
                var accessor = this.conversationState.CreateProperty<ConversationReference>(nameof(ConversationReference));
    ...
    protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken = default)
    {
        ...
        if (participant.Meeting.Role == "Organizer")
        {
            ...
            else
            {
                ...
                {
                    var arguments = await this.openAIService.GetConversationPropertiesOptionsAsync(command, cancellationToken);
    ...
    )
    {
        ...
        var accessor = this.conversationState.CreateProperty<ConversationProperties>(nameof(ConversationProperties));
        var conversationProperties = await accessor.GetAsync(
            turnContext,
            () => new(),
            cancellationToken
        );
        var meetingInfo = await TeamsInfo.GetMeetingInfoAsync(turnContext, cancellationToken: cancellationToken);
        conversationProperties.InMeeting = true;
        conversationProperties.StartMeetingSended = false;
        conversationProperties.EndMeetingSended = false;
        conversationProperties.ScheduledStartTime = meetingInfo.Details.ScheduledStartTime;
        conversationProperties.ScheduledEndTime = meetingInfo.Details.ScheduledEndTime;
        await accessor.SetAsync(
            turnContext,
    ...
    )
    {
        ...
        conversationProperties.InMeeting = false;
        conversationProperties.StartMeetingSended = false;
        conversationProperties.EndMeetingSended = false;
        conversationProperties.ScheduledStartTime = null;
        conversationProperties.ScheduledEndTime = null;
        await accessor.SetAsync(
            turnContext,
            conversationProperties,
            cancellationToken: cancellationToken
